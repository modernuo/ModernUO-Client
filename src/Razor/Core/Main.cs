...
namespace Assistant
{
    public partial class Engine
    {
        ...
        public static string Version
        {
            get
            {
                if (m_Version == null)
                {
                    Version v = new Version(1, 0, 0, 0);// Assembly.GetCallingAssembly().GetName().Version;
 ...
 new UnhandledExceptionEventHandler( CurrentDomain_UnhandledException );
        ...
        public static string RootPath =>
            _rootPath ?? (_rootPath = Path.GetDirectoryName($"{AppContext.BaseDirectory}\\Data\\Plugins\\Razor\\"));
        ...
        {
            //Reflection cannot be used for NativeAOT build
            BuyAgent.Initialize();
            ...
            Commands.Initialize();
            MemoryHelperThinggie.Initialize();
            ScreenCapManager.Initialize();
            Spell.Initialize();
            Targeting.Initialize();
            WaypointManager.Initialize();
            DeathFilter.Initialize();
            LightFilter.Initialize();
            LocMessageFilter.Initialize();
            SoundFilter.Initialize();
            StaffItemFilter.Initialize();
            VetRewardGumpFilter.Initialize();
            WeatherFilter.Initialize();
            CounterHotKeys.Initialize();
            MessageManager.Initialize();
            CooldownManager.Initialize();
            SystemMessages.Initialize();
            TextFilterManager.Initialize();
            HotKey.Initialize();
