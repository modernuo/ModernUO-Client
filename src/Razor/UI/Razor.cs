...
namespace Assistant
{
    public partial class MainForm
    {
        ...
        private Version m_Ver = new Version(1, 0, 0, 0); //System.Reflection.Assembly.GetCallingAssembly().GetName().Version;
        ...
        private void skillCopySel_Click(object sender, System.EventArgs e)
        {
            ...
            if (sb.Length > 0)
                ClassicUO.Utility.StringHelper.SetClipboardText(sb.ToString());
        }
        ...
        private void newMacro_Click(object sender, System.EventArgs e)
        {
            if (InputBox.Show(this, Language.GetString(LocString.NewMacro), Language.GetString(LocString.EnterAName)))
            {
                ...
                try
                {
                    ...
                    var clipboard = ClassicUO.Utility.StringHelper.GetClipboardText(false);
                    if (clipboard.Contains("Assistant.Macros."))
                    {
                        ...
                        {
                            string[] macroCommands = clipboard
                                .Split(new[] {"\r\n", "\r", "\n"}, StringSplitOptions.None);

                            File.WriteAllLines(path, macroCommands);

                            ClassicUO.Utility.StringHelper.SetClipboardText(string.Empty);
                        }
                        else
                        {
        ...
        private void Macro_CopyToClipboard(object sender, EventArgs args)
        {
            ...
            try
            {
                ...
                ClassicUO.Utility.StringHelper.SetClipboardText(string.Join(Environment.NewLine, macroLines));
            }
            catch
            {
        ...
        {
            //Image clipboard functionality is not supported for NativeAOT
            return;
        ...
        private void OnAgentImport(object sender, System.EventArgs e)
        {
            ...
            var clipboard = ClassicUO.Utility.StringHelper.GetClipboardText(false);
            if (!clipboard.Contains("!Razor.Agents"))
            {
                return;
            }

            List<string> items = clipboard
                .Split(new[] { "\r\n", "\r", "\n" }, StringSplitOptions.None).ToList();

            if (agent is OrganizerAgent organizerAgent && clipboard.Contains("!Razor.Agents.Organizer"))
            {
                items.RemoveAt(0);
            ...
            else if (agent is BuyAgent buyAgent && clipboard.Contains("!Razor.Agents.Buy"))
            {
                items.RemoveAt(0);
            ...
            else if (agent is RestockAgent restockAgent && clipboard.Contains("!Razor.Agents.Restock"))
            {
                items.RemoveAt(0);
            ...
            else if (agent is ScavengerAgent scavengerAgent && clipboard.Contains("!Razor.Agents.Scavenger"))
            {
                items.RemoveAt(0);
            ...
            else if (agent is SellAgent sellAgent && clipboard.Contains("!Razor.Agents.Sell"))
            {
                items.RemoveAt(0);
        ...
        private void onImportFriends(object sender, System.EventArgs e)
        {
            ...
            {
                var clipboard = ClassicUO.Utility.StringHelper.GetClipboardText(false);
                if (clipboard.Contains("!Razor.Friends.Import"))
                {
                    List<string> friendsImport = clipboard
                        .Split(new[] {"\r\n", "\r", "\n"}, StringSplitOptions.None).ToList();
        ...
        private void CopyScriptToClipboard(object sender, EventArgs e)
        {
            ...
            try
            {
                ClassicUO.Utility.StringHelper.SetClipboardText(string.Join(Environment.NewLine, script.Lines));
            }
            catch
            {
                MessageBox.Show(this, Language.GetString(LocString.ReadError), "Copy Error", MessageBoxButtons.OK,
        ...
        private void onImportWaypoints(object sender, EventArgs e)
        {
            ...
            {
                var clipboard = ClassicUO.Utility.StringHelper.GetClipboardText(false);
                if (clipboard.Contains("!Razor.Waypoints.Import"))
                {
                    List<string> waypointImport = clipboard
                        .Split(new[] { "\r\n", "\r", "\n" }, StringSplitOptions.None).ToList();
        ...
        private void ItemAddCommand(object sender, EventArgs e)
        {
            ...
            if (itemTree.SelectedNode != null)
            {
                ...
                {
                    ClassicUO.Utility.StringHelper.SetClipboardText(command);
                }
            }
        }
