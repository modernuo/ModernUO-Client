            ...
            foreach (XmlElement el in root.GetElementsByTagName("property"))
            {
                try
                {
                    ...
                    else
                    {
                        ...
                        if (type == null)
                        {
                            if (type == typeof(bool) && bool.TryParse(val, out var boolResult))
                            {
                                m_Props[name] = boolResult;
                            }
                            else if (type == typeof(int) && int.TryParse(val, out var intResult))
                            {
                                m_Props[name] = intResult;
                            }
                            else if (type == typeof(string))
                            {
                                m_Props[name] = val;
                            }
                            else if (type == typeof(double) && double.TryParse(val, out var doubleResult))
                            {
                                m_Props[name] = doubleResult;
                            }
                            else if (type == typeof(double) && long.TryParse(val, out var longResult))
                            {
                                m_Props[name] = longResult;
                            }
                            else if (type == typeof(byte) && byte.TryParse(val, out var byteResult))
                            {
                                m_Props[name] = byteResult;
                            }
                            else if (type == typeof(decimal) && byte.TryParse(val, out var decimalResult))
                            {
                                m_Props[name] = decimalResult;
                            }
                            else
                            {
                                throw new Exception($"Razor profile loading: {name} {typeStr} not supported");
                            }
                        }
